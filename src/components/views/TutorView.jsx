...
const cellTitle = (a, b, fact) =>
  `${a} × ${b}: streak ${fact.correctStreak}, seen ${fact.seen}` +
  (fact.avgCorrectMs != null ? `, avg ${seconds(fact.avgCorrectMs)}` : '');
...
export const TutorView = ({ tutorKey }) => {
  ...
  const topicRows = Object.entries(report.topics).filter(([, t]) => t.attempted > 0);
  ...
  return frame(
    <>
      ...
      <div style={panel}>
        <span style={labelStyle}>SLOWEST SOLID FACTS</span>
        {report.slowestSolid.length === 0 ? (
          <div style={{ fontSize: 13, color: '#7a8571', fontStyle: 'italic' }}>
            Not enough correct answers yet to rank slow facts.
          </div>
        ) : (
          report.slowestSolid.map((item) => (
            <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '4px 0' }}>
              <span style={{ fontSize: 14, color: '#e8dcc0' }}>{item.label}</span>
              <span style={{ fontSize: 11, color: '#7a8571' }}>{seconds(item.avgCorrectMs)} average</span>
            </div>
          ))
        )}
        <div style={noteStyle}>
          Multiplication facts she gets right but slowly (correct at least twice,
          past the missed stage). These are worth drilling for speed.
        </div>
      </div>

      <div style={panel}>
        <span style={labelStyle}>BY TOPIC</span>
        {topicRows.length === 0 ? (
          <div style={{ fontSize: 13, color: '#7a8571', fontStyle: 'italic' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, color: '#c8c0a8' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #2a3329', color: '#7a8571' }}>
                <th style={{ textAlign: 'left', padding: '4px 8px 4px 0', fontWeight: 400 }}>Topic</th>
                <th style={{ textAlign: 'right', padding: '4px 8px', fontWeight: 400 }}>Correct</th>
                <th style={{ textAlign: 'right', padding: '4px 8px', fontWeight: 400 }}>Accuracy</th>
                <th style={{ textAlign: 'right', padding: '4px 0 4px 8px', fontWeight: 400 }}>Avg time</th>
              </tr>
            </thead>
            <tbody>
              {topicRows.map(([topic, t]) => (
                <tr key={topic} style={{ borderBottom: '1px solid #2a3329' }}>
                  <td style={{ padding: '6px 8px 6px 0', color: '#a39d88' }}>{TOPIC_NAMES[topic] || topic}</td>
                  <td style={{ textAlign: 'right', padding: '6px 8px' }}>{t.correct} / {t.attempted}</td>
                  <td style={{ textAlign: 'right', padding: '6px 8px' }}>
                    {t.accuracy === null ? '-' : `${Math.round(t.accuracy * 100)}%`}
                  </td>
                  <td style={{ textAlign: 'right', padding: '6px 0 6px 8px' }}>{seconds(t.avgMs)}</td>
                </tr>
              ))}
            </tbody>
          </table>
