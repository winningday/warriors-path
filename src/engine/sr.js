...
export const applySRResult = (entry, isCorrect, elapsedMs, kind, personalFastMs, now = Date.now()) => {
  const next = { ...entry };
  ...
  if (!isCorrect) {
    ...
    // A miss forfeits any pending victory lap.
  ...
  if (shouldPromote) {
    const from = SR_BUCKET_ORDER.indexOf(entry.bucket);
    const idx = Math.min(SR_BUCKET_ORDER.length - 1, from + 1);
    next.bucket = SR_BUCKET_ORDER[idx];
    // A real bucket rise stamps promotedAt so selectFact can offer a
    // "victory lap": the freshly-won fact comes back soon while the win
    // still feels good.
    if (idx > from) next.promotedAt = now;
  }
  return next;
};
...
// Victory laps (v15.2): shortly after a fact is promoted, give it one easy
// re-appearance so the win gets felt instead of the fact vanishing into a
// rarer bucket.
